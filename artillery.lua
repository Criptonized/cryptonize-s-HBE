		...
		ctx:Connect(RunService.RenderStepped, function()
			...
			if Toggles.artTrajectory.Value and targetPos then
				...
				for i = 0, RING_N do
					...
					local wp = targetPos + Vector3.new(math.cos(a) * R, 0, math.sin(a) * R)
			...
			if Toggles.artTargetCam.Value and targetPos then
				camActive = true
				local root = art:FindFirstChild("Root") or art:FindFirstChild("Muzzle", true)
				local back = Vector3.new(0, 0, 0)
				if root then
					local dir = (root.Position - targetPos)
					dir = Vector3.new(dir.X, 0, dir.Z)
					if dir.Magnitude > 0.01 then back = dir.Unit * Options.artCamBack.Value end
				end
