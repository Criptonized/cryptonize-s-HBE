...
return {
	...
	local function resolveFields(tool)
		-- NO caching: re-resolve fresh every call so a respawn / new gun is ALWAYS handled.
		-- The per-tool cache is what made inf-ammo stop after death -- it held the dead gun's
		-- value objects. The strategy fall-through returns on the first hit, so this is cheap
		-- (usually a single descendant scan; ValueNames catches the TREK DoubleConstrained ammo).
		for _, s in ipairs(STRATS) do
			local fields = s.fn(tool)
			if #fields > 0 then return { fields = fields, how = s.name } end
		end
		local learned = stratLearning(tool)
		if #learned > 0 then return { fields = learned, how = "Learned" } end
		return nil
	end
	...
	local function refillTool(tool)
		...
		-- Show method + LIVE field count so you can see it re-detect the new gun after a death.
		local label = res.how .. " (" .. #res.fields .. " on " .. tool.Name .. ")"
		if label ~= lastHow then
			lastHow = label
			pcall(function() detLabel:SetText("Detection: " .. label) end)
		end
	end
