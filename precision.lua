-- CryptsHBE plugin: Precision HBE (standalone single-target extender + resolver)
-- Externalized from the core. Self-contained; uses globals (Toggles/Options/Library,
-- getgenv().DrawingFallback, getgenv().CryptsHBE = Bridge) + the ctx sandbox.
...
return {
	...
	load = function(ctx)
		...
		-- Save into the shared workspace/CryptsHBE/ folder (same place the core's
		...
		howGroup:AddLabel(
			"Enlarges ONE target's hitbox (not\n" ..
			"everyone) so your shots/melee connect.\n" ..
			"Works even with the Main Master Toggle\n" ..
			"off.\n\n" ..
			"  1. Enable Precision HBE.\n" ..
			"  2. Leave 'Auto-Select Target' on +\n" ..
			"     pick a Resolver (Crosshair /\n" ..
			"     Distance / Lowest Health), or turn\n" ..
			"     it off and press Cycle Target\n" ..
			"     (default T).\n" ..
			"  3. Set Base Hitbox Size + Parts.\n\n" ..
			"KEEP Parts on Head/Torso -- extending\n" ..
			"HumanoidRootPart FREEZES the target on\n" ..
			"your screen (Info warns [FROZEN?]).\n" ..
			"Dynamic Scaling grows the box at range;\n" ..
			"Transparency 1 = invisible. Save/Load\n" ..
			"Config persists your setup.",
			true)
