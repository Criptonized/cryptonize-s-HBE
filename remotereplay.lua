-- FurryHBE plugin: Remote Replay (manual RemoteEvent replay at the nearest target)
...
return {
	...
	local rrConn = RunService.Heartbeat:Connect(function()
		...
		howG:AddLabel(
			"For games where SHOOTING = firing a RemoteEvent (not touch). You fire the game's own\n" ..
			"damage remote at the nearest enemy -- no aiming, no hooks.\n\n" ..
			"  1. Refresh Remotes (lists every RemoteEvent).\n" ..
			"  2. Pick the one that deals damage. Hints: names like FireBullet/Hit/Damage/Shoot;\n" ..
			"     use Calibrate -> Deep-Dump Held Weapon to see the gun's remotes.\n" ..
			"  3. Set Argument to what the remote expects (Target Character / Player / HRP / Position).\n" ..
			"  4. Fire at Nearest (one shot) or Auto-Fire (aura). Set Range + Rate.\n\n" ..
			"Nothing happens? Wrong remote or wrong Argument -- try another. Honeypot remotes mapped\n" ..
			"by Phantom Recon are skipped automatically.",
			true)
