	...
	load = function(ctx)
		...
		g:AddButton("Save Log to file", function()
			...
			pcall(function()
				...
				if writefile then writefile("CryptsHBE/remotesniff_" .. tostring(game.PlaceId) .. ".txt", table.concat(lines, "\n")) end
			end)
			Library:Notify("Saved -> workspace/CryptsHBE/remotesniff_" .. tostring(game.PlaceId) .. ".txt")
