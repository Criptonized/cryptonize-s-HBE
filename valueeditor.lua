...
return {
	...
	load = function(ctx)
		...
		local function objAt(px, py)
			...
			for _, gui in ipairs({ pg, game:GetService("CoreGui") }) do
				if gui then
					pcall(function()
		...
		-- ===== Pick from HUD =====
		pickG:AddLabel("Hover/click a NUMBER on your screen (ammo, speed,\nmoney) -- finds every value equal to it.", true)
		pickG:AddToggle("veInspect", { Text = "Inspect Mode (hover)", Default = false, Tooltip = "Outlines the HUD element under your cursor + shows its path/number, like a browser inspector. Then hit 'Capture Hovered'. (Default: OFF)" }); ctx:Control("veInspect")
		local lblHover = pickG:AddLabel("Hover: -", true)
		local lastHover = nil
		pickG:AddButton("Capture Hovered", function()
			if not lastHover then Library:Notify("Nothing hovered (turn on Inspect Mode + point at a number)"); return end
			local num = numFromText(lastHover.Text)
			if not num then Library:Notify("Hovered element has no number"); return end
			searchNumber(num, lastHover)
		end):AddToolTip("Search for the number in the currently-hovered element (use with Inspect Mode).")
		pickG:AddButton("Pick HUD Number (click)", function()
			...
			conn = UserInputService.InputBegan:Connect(function(i)
				...
				if not o then Library:Notify("No number under the cursor"); return end
				local num = numFromText(o.Text)
				if not num then Library:Notify("No number under the cursor"); return end
				searchNumber(num, o)
			end)
		end):AddToolTip("Arm, then click a HUD number.")
		...
		ctx:Connect(RunService.RenderStepped, function()
			...
			local now = tick(); if now - lastInspect < 0.05 then return end; lastInspect = now
			local pos = UserInputService:GetMouseLocation()
			local o = objAt(pos.X, pos.Y)
			lastHover = o
			if o then
				boxTo(hoverBox, o)
				pcall(function()
					hoverTag.Text = " " .. o.Name .. "  = " .. tostring(numFromText(o.Text)) .. " "
					local ap = o.AbsolutePosition
					hoverTag.Position = UDim2.fromOffset(ap.X, math.max(0, ap.Y - 18))
					hoverTag.Visible = true
				end)
				pcall(function() lblHover:SetText("Hover: " .. o.Name .. " = " .. tostring(numFromText(o.Text))) end)
			else
				hoverBox.Visible = false; hoverTag.Visible = false
				pcall(function() lblHover:SetText("Hover: (no number under cursor)") end)
			end
		end)
		...
		howG:AddLabel(
			...
			"  - 'Pick HUD Number', then click it. OR\n" ..
